...
import {  SectionDataType, StoreType } from '@hrms-workspace/frontend/types';
...
export function Section() {
  ...
  const sectionData = Store((state: StoreType) => state.allSectionData);
  const setSectionData = Store((state: StoreType) => state.setSectionData);
  ...
  const columnHelper = createColumnHelper<SectionDataType>();
  ...
  const columns = [
    ...
    columnHelper.accessor('section_en', {
      header: 'Section Name (EN)',
      cell: (info) => info.renderValue(),
      footer: (info) => info.column.id,
    }),
    columnHelper.accessor('section_np', {
      header: 'Section Name (NP)',
      cell: (info) => info.renderValue(),
      footer: (info) => info.column.id,
    }),
    columnHelper.accessor('secCode', {
      header: 'Section Code',
      cell: (info) => info.renderValue(),
      footer: (info) => info.column.id,
    }),
    columnHelper.accessor('sectionHead', {
      header: 'Section Head',
      cell: (info) => info.renderValue(),
      footer: (info) => info.column.id,
    }),
  ...
  const { refetch } = useQuery(
    ['sectionData', pageNumber, pageSize,searchData],
    () =>
      request.section.getAllSection(
  ...
  const deleteSection = useMutation(
    (id: number) => {
      return request.section.deleteSection(accessToken,id);
    },
    {
      onSuccess() {
  ...
  return (
    <div>
                      ...
                      <div className="flex justify-center space-x-[8px] mt-[8px]">
                        <div className="bg-[#2563EB] text-white-12 p-[4px] rounded-[4px]">
                          <Pencil1Icon
                            ...
                            onClick={() =>
                              navigate(`/addSection/${row.original.secId}`)
                            }
                          />
                        </div>
                        ...
                        <div className="bg-[#dc2626] text-white-12 p-[4px] rounded-[4px]">
                          <BsFillTrashFill
                            className="cursor-pointer w-[16px] h-[16px]"
                            onClick={() =>
                              deleteSection.mutateAsync(row.original.secId)
                            }
                          />
                        </div>
                      </div>
      ...
      <div className="flex items-center justify-between mb-10 w-full px-10 mt-[24px]">
        <div>
          <TablePagination
            storeData={sectionData}
            pageCount={sectionData.totalPages}
